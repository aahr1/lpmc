...
cap log close
log using "`c(pwd)'\out10\mc_log_p`thep'_alter`thealternate'_design`thedesign'", text replace
...
foreach s in 0.5 1 2 3 5 {
	...
	simulate , reps(300): mysim, obs(400) tobs(200) p(100) design(1) sigma(1) alter(`thealternate')
	save "`c(pwd)'\out10\mc_p`thep'_alter`thealternate'_design`thedesign'_`mc'.dta", replace
	local mc = `mc' + 1	
