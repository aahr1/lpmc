...
global outputpath "`c(pwd)'\do_files\mc7b"
...
order design theta delta s dim sigma p obs iter dim /// 
...
keep design theta delta s dim sigma p obs iter *_str
...
sort design dimix sigma

texsave dim sigma AIC-cvel cvr Ora using "do_files\tex\MC_results_7b_design1.tex" ///
						if design == 1, ///
						replace hlines(5(5)25) size(scriptsize) nofix
				
texsave dim sigma AIC-cvel cvr using "do_files\tex\MC_results_7b_design2.tex" ///
						if design == 2, ///
						replace hlines(5(5)15) size(scriptsize) nofix
