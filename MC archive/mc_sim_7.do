...
cap log close
log using "`c(pwd)'\Do_files\mc7b\mc7b_log", text replace
...
local mc=1
foreach te in 0.7 0.9 {
foreach s in 0.5 1 2 3 5 {
	
	simulate , reps(500): mysim, obs(400) tobs(200) p(100) design(1) sigma(`s') theta(`te')
	save "`c(pwd)'\Do_files\mc7b\mc_`mc'.dta", replace
	local mc = `mc' + 1	
	
	simulate , reps(500): mysim, obs(400) tobs(200) p(100) design(2) sigma(`s') theta(`te')
	save "`c(pwd)'\Do_files\mc7b\mc_`mc'.dta", replace
	local mc = `mc' + 1	
