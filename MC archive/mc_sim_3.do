...
//global outputpath "C:\Users\Achim.Ahrens\Dropbox\StataLasso\Current\Do_files\mc6"
...
program define getrmse , rclass
	syntax varlist(min=2 max=2) , esample(varlist min=1 max=1)
...
program define mysim, rclass
	...
	************* generate data end ********************************************
	...
	ereturn clear
	cvlasso y x1-x`p' if `esample', ///
				lopt postest nfolds(5) unitl nopath alpha(0.1 0.5 0.9)
...
//set seed 123
set seed 456
//mysim, obs(300) p(100) tobs(200)

local mc=21
foreach s in 0.5 1 2 3 5 {
	
	simulate , reps(50): mysim, obs(400) tobs(200) p(100) design(1) sigma(`s') theta(.7)
	save "Do_files\mc6\mc_`mc'.dta", replace
	local mc = `mc' + 1	
	
	simulate , reps(50): mysim, obs(400) tobs(200) p(100) design(2) sigma(`s') theta(.7)
	save "Do_files\mc6\mc_`mc'.dta", replace
	local mc = `mc' + 1	
