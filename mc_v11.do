...
foreach s in 0.5 1 2 3 5 {
	...
	simulate , reps(2): mysim, obs(400) tobs(200) p(`thep') design(`thedesign') sigma(`s') alter(`thealternate') ///
						theta(0)
	save "`c(pwd)'\out11\mc_p`thep'_alter`thealternate'_design`thedesign'_`mc'_zerotheta.dta", replace
	local mc = `mc' + 1	
