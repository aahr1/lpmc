* aa 
* date of last run started: 13nov2018
clear
...
cap log close
log using "`c(pwd)'\out12\mc_log_p`thep'_alter`thealternate'_design`thedesign'", text replace
...
foreach s in 0.5 1 2 3 5 {
	...
	simulate , reps(1000): mysim, obs(400) tobs(200) p(`thep') design(`thedesign') sigma(`s') alter(`thealternate')
	save "`c(pwd)'\out12\mc_p`thep'_alter`thealternate'_design`thedesign'_`mc'.dta", replace
	local mc = `mc' + 1	
